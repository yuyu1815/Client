...
//! Candidate source only: production integration/compilation is owned
//! elsewhere.
use glam::{Mat4, Vec3};

pub const ACTIVATION_TICKS: u32 = 40;
/// Vanilla ScreenEffectRenderer submits packed light 15728880 (block+sky
/// fullbright).
pub const ACTIVATION_PACKED_LIGHT: u32 = 15_728_880;
/// Pomme's item_entity.frag has no lightmap sample; fullbright maps to
/// multiplier 1.
...
pub struct ActivationPose {
    /// ScreenEffect pose followed by the ItemDisplayContext::FIXED item
    /// transform.
    pub model: Mat4,
...
/// existing ItemVertex mesh is already centered [-.5,.5], so do not subtract
/// .5.
...
/// `hud_fov` comes from that camera state; dimensions are window framebuffer
/// pixels.
